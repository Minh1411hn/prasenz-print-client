...
echo -e "${YELLOW}[1/5] Checking macOS compilation tools...${NC}"
command -v swiftc >/dev/null 2>&1 || { echo -e "${RED}❌ Command Line Tools need to be installed: xcode-select --install${NC}"; exit 1; }
command -v lipo >/dev/null 2>&1 || { echo -e "${RED}❌ System lipo tool is required${NC}"; exit 1; }
echo -e "${GREEN}✅ swiftc OK | lipo OK${NC}"
echo

# Step 2: Download cloudflared binaries if not present
echo -e "${YELLOW}[2/5] Checking Cloudflare Tunnel binaries...${NC}"
mkdir -p "$BIN_DIR"

CF_BASE_URL="https://github.com/cloudflare/cloudflared/releases/latest/download"

if [ ! -f "$BIN_DIR/cloudflared-intel" ]; then
  echo -e "  ${BLUE}⬇️  Downloading cloudflared for Intel (amd64)...${NC}"
  ...
  echo -e "  ${GREEN}✅ cloudflared-intel is ready${NC}"
else
  echo -e "  ${GREEN}✅ cloudflared-intel already exists${NC}"
fi

if [ ! -f "$BIN_DIR/cloudflared-silicon" ]; then
  echo -e "  ${BLUE}⬇️  Downloading cloudflared for Apple Silicon (arm64)...${NC}"
  ...
  echo -e "  ${GREEN}✅ cloudflared-silicon is ready${NC}"
else
  echo -e "  ${GREEN}✅ cloudflared-silicon already exists${NC}"
fi
...
echo -e "${YELLOW}[3/5] Compiling Swift GUI and background HTTP Server...${NC}"
mkdir -p "$DIST_DIR"
SDK_PATH=$(xcrun --show-sdk-path --sdk macosx)

echo -e "  ${BLUE}⚙️  Compiling for Intel (x64)...${NC}"
...
echo -e "  ${BLUE}⚙️  Compiling for Apple Silicon (arm64)...${NC}"
swiftc -O -sdk "$SDK_PATH" -target arm64-apple-macosx11.0 \
  src-swift/main.swift src-swift/AppDelegate.swift src-swift/HttpServer.swift \
  -o "$DIST_DIR/PrasenzPrinter-macos-arm64"

echo -e "  ${BLUE}⚙️  Merging 2 architectures into a Universal Binary...${NC}"
lipo -create \
...
echo -e "${YELLOW}[4/5] Assembling complete .app bundle structure...${NC}"
rm -rf "$APP_BUNDLE"
...
echo -e "${GREEN}✅ .app bundle assembled successfully${NC}"
echo

# Step 5: Finished
echo -e "${BLUE}=====================================================${NC}"
echo -e "${GREEN}🎉 NATIVE SWIFT APP BUILD SUCCESSFUL!${NC}"
echo -e "${BLUE}=====================================================${NC}"
echo -e "  📦 App Bundle: ${APP_BUNDLE}"
echo -e "  📏 Size: $(du -sh "$APP_BUNDLE" | cut -f1) (Reduced by more than 4x!)"
echo
echo -e "  👉 Just drag and drop the file ${YELLOW}dist/PrasenzPrinter.app${NC} into the ${YELLOW}/Applications${NC} folder"
echo -e "  👉 Then double-click the application to activate running in the background on the Menu Bar."
echo
